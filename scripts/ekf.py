...
class EKF(eqx.Module):
    ...
    def propagate(
        ...
        df_dx = jax.jacfwd(self.f_sys, argnums=0)(mu_prev, u) # numerator/Jacobian convention, not gradient/denominator
        nx = mu_prev.shape[0]
        if not self.is_discrete:
...
def main() -> None:
    ...
    # call filter
    my_ekf = EKF(f_sys=f, h_sys=h, Q=Q, R=R, is_discrete=True)
