...
filter_type = 'ekf'; % choose one among 'none', 'ekf', and 'pf'
...
function [mean_position_error, anees] = localize(env, policy, filt, x0, num_steps, do_plot)
    ...
    if do_plot
        ...
        if ~isempty(filt)
            plot(states_filter(:, 1), states_filter(:, 2), 'r', 'LineWidth', 1.5, 'LineStyle', '--');
        end
        drawnow;
    end
end
...
function filt = ekf_update(filt, env, u, z, marker_id)
    % 1. Prediction Step
    mu_bar = forward_model(filt.mu, u);
    
    G_t = G(env, filt.mu, u);
    V_t = V(env, filt.mu, u);
    M_t = noise_from_motion(u, filt.alphas);
    
    Sigma_bar = G_t * filt.sigma * G_t' + V_t * M_t * V_t';
    
    % 2. Correction Step
    H_t = H(env, mu_bar, marker_id);
    Q_t = filt.beta;
    
    S_t = H_t * Sigma_bar * H_t' + Q_t;
    K_t = Sigma_bar * H_t' / S_t;
    
    z_expected = observe(env, mu_bar, marker_id);
    innovation = minimized_angle(z - z_expected);
    
    filt.mu = mu_bar + K_t * innovation;
    filt.mu(3) = minimized_angle(filt.mu(3)); % Wrap final heading
    
    I = eye(3);
    filt.sigma = (I - K_t * H_t) * Sigma_bar;
end
...
function filt = pf_update(filt, env, u, z, marker_id)
    ...
    % 2. Weight calculation based on measurement likelihood
    num_particles = filt.num_particles;
    for i = 1:num_particles
        z_expected = observe(env, filt.particles(i, :)', marker_id);
        innov = minimized_angle(z - z_expected);
        % Gaussian PDF evaluation for the weight
        filt.weights(i) = exp(-0.5 * (innov^2) / filt.beta);
    end
    
    % Normalize weights to sum to 1
    filt.weights = filt.weights / sum(filt.weights);
    
    % 3. Resample
    filt.particles = pf_resample(filt.particles, filt.weights);
    
    % Extract mean and variance for plotting/metrics
    [filt.mu, filt.sigma] = pf_mean_and_variance(filt.particles);
end

function new_particles = pf_move_particles(filt, env, u)
    num_particles = size(filt.particles, 1);
    new_particles = zeros(num_particles, 3);
    
    % Propagate each particle through the motion model with injected noise
    for i = 1:num_particles
        u_noisy = sample_noisy_action(u, filt.alphas);
        new_particles(i, :) = forward_model(filt.particles(i, :)', u_noisy)';
    end
end
...
function grad_G = G(env, x, u)
    ...
    grad_G = eye(3); 
    grad_G(1, 3) = -trans * sin(prev_theta + rot1);
    grad_G(2, 3) = trans * cos(prev_theta + rot1);
end

function grad_V = V(env, x, u)
    % Jacobian of the motion model w.r.t the control input
    prev_theta = x(3);
    rot1 = u(1);
    trans = u(2);
    
    grad_V = zeros(3, 3);
    grad_V(1, 1) = -trans * sin(prev_theta + rot1);
    grad_V(1, 2) = cos(prev_theta + rot1);
    
    grad_V(2, 1) = trans * cos(prev_theta + rot1);
    grad_V(2, 2) = sin(prev_theta + rot1);
...
function grad_H = H(env, x, marker_id)
    % Jacobian of the measurement model w.r.t the state
    lx = env.MARKER_POS(1, marker_id);
    ly = env.MARKER_POS(2, marker_id);
